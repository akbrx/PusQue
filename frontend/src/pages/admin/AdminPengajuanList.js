class AdminPengajuanList extends HTMLElement {
    ...
    async fetchAntrian() {
      try {
        const res = await fetch('https://serverpusque-production.up.railway.app/antrian', {
          credentials: 'include'
        });
      ...
      this.querySelectorAll('button[data-action="kembalikan"]').forEach(btn => {
        btn.addEventListener('click', async (e) => {
          ...
          if (confirm('Kembalikan antrian ini ke status "menunggu acc admin"?')) {
            try {
              const res = await fetch(`https://serverpusque-production.up.railway.app/antrian/${id}/kembalikan`, {
                method: 'PATCH',
                credentials: 'include'
              });
              if (res.ok) {
