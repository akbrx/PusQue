class FeedbackPage extends HTMLElement {
  async connectedCallback() {
    ...
    try {
      ...
      const res = await fetch('https://serverpusque-production.up.railway.app/antrian/riwayat', { credentials: 'include' });
      riwayat = await res.json();
      // Ambil feedback yang sudah pernah diberikan user
      const resFb = await fetch('https://serverpusque-production.up.railway.app/feedback/user', { credentials: 'include' });
  ...
  render(riwayat, feedbacks) {
    ...
    this.querySelectorAll('.feedback-form').forEach(form => {
      form.addEventListener('submit', async (e) => {
        ...
        try {
          const res = await fetch('https://serverpusque-production.up.railway.app/feedback', {
            method: 'POST',
